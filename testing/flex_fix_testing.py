...
def flex_fix_test():
    ...
    servers = [
        ...
        Server("Zulu", storage_capacity=385, computation_capacity=90, bandwidth_capacity=250)
    ]
    ...
    plot_allocation_results(jobs, servers, "Optimal flexible resource requirement allocation",
    ...
    plot_allocation_results(fixed_jobs, servers, "Optimal fixed resource requirement allocation",
                            save_format=ImageFormat.BOTH)

    print('\n\nGreedy')
    reset_model(jobs, servers)
    greedy_results = greedy_algorithm(jobs, servers, ResourceSum(), SumResources(), SumPercentage())
    plot_allocation_results(jobs, servers, "Greedy flexible resource requirement allocation",
                            ImageFormat.BOTH)

    print('Optimal, Fixed and Greedy results')
    print_results({'optimal': optimal_results, 'fixed': fixed_results, 'greedy': greedy_results})

...
def mutated_task_test(model_dist: ModelDist, repeat: int, repeats: int = 50, price_change: int = 2,
    ...
    """
    print(f'Mutate tasks and servers with iterative auctions for '
    ...
    for _ in tqdm(range(repeats)):
        ...
        for _ in range(mutate_repeats):
            ...
            mutant_result = decentralised_iterative_auction(tasks, servers, time_limit, initial_cost)
...
def all_task_mutations_test(model_dist: ModelDist, repeat: int, num_mutated_tasks=5, percent: float = 0.15,
    ...
    print(f"All mutation auction tests with {model_dist.num_tasks} tasks and {model_dist.num_servers} servers, "
          f"time limit {time_limit} sec and initial cost {initial_cost(Task('', 0, 0, 0, 0, 0))} ")
                    ...
                    for value in range(int(task.value * negative_percent), task.value + 1):
                        for deadline in range(int(task.deadline * negative_percent), task.deadline + 1):
                            ...
                            if result is not None:
                                ...
                                print(result.store(difference=task_diff(mutant_task, task), mutant_value=mutant_task))
...
if __name__ == "__main__":
    ...
    mutated_task_test(loaded_model_dist, args['repeat'])

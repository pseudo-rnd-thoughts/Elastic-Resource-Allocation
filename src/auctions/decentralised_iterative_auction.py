...
def evaluate_optimal_task_price(new_task: Task, server: Server, time_limit: int, initial_cost: int,
    ...
    model = CpoModel(f'Task {new_task.name} Price')
    ...
    loading_speed = {task: model.integer_var(min=1, max=server.bandwidth_capacity - 1,
                                             name=f'Task {task.name} loading speed') for task in tasks}
    compute_speed = {task: model.integer_var(min=1, max=server.computation_capacity,
                                             name=f'Task {task.name} compute speed') for task in tasks}
    sending_speed = {task: model.integer_var(min=1, max=server.bandwidth_capacity - 1,
                                             name=f'Task {task.name} sending speed') for task in tasks}
    # Create all of the allocation variables however only on the currently allocated tasks
    allocation = {task: model.binary_var(name=f'Task {task.name} allocated') for task in server.allocated_tasks}
    ...
    task_price = server.revenue - max_server_profit + server.price_change
    ...
    if debug_results:
        print(f'Sever: {server.name} - Max server profit: {model_solution.get_objective_values()[0]}, '
              f'prior server total price: {server.revenue} therefore task price: {task_price}')
...
def allocate_tasks(task_price: float, new_task: Task, server: Server,
    ...
    for task, allocated in allocation.items():
        ...
        if debug_allocations:
            print(f"Task {task.name} is {'allocated' if allocation[task] else 'unallocated'} to server {server.name} "
                  f"with loading {loading[task]}, compute {compute[task]} and sending {sending[task]}")
    if debug_result:
        print(f'Server {server.name}\'s total price: {server.revenue}')

    return messages


def decentralised_iterative_auction(tasks: List[Task], servers: List[Server], time_limit: int,
    ...
    while len(unallocated_tasks):
        ...
        if task_price <= task.value:
            ...
            print(f'Removing Task {task.name} from the unallocated task as the min price is {task_price} '
                  f'and task value is {task.value}')
        unallocated_tasks.remove(task)
    ...
    if debug_results:
        print(f'It is finished, number of iterations: {iterations} '
              f'with social welfare: {sum(server.revenue for server in servers)}')
        for server in servers:
            print(f'Server {server.name}: total revenue - {server.revenue}')
            print("\tTasks - {}".format(', '.join([f'{task.name}: £{task.price}' for task in server.allocated_tasks])))

    return Result('Iterative', tasks, servers, time() - start_time, individual_compute_time=time_limit, show_money=True,
